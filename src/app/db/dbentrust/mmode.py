...
class MMode(MemObject):
    ...
    def update_mem_only(self, mapping):
        ...
        props = {'data': data.get('data'), '_time': ntime}  # 不同步sql,所以不写入redis,不修改状态
...
#         return list(set(dbkeylist))
    ...
    def delObjListByIdlist(self, pklist=None):
        '''
        根据Id列表删除缓存,不删除数据库(注:仅用于更新缓存)
        '''
        logger.debug('delete %s memcahce' % self._name)
        if not pklist:
            pklist = []
        for pk in pklist:
            mm = MMode(self._name + ':%s' % pk, self._pk)
            ...
            data = mm.get('data')
            if data:
                mm.delete_mem_only()  # 删除缓存
                fk = data.get(self._fk, 0)  # 删除通过外键查找的主键列表
                name = '%s_fk:%s' % (self._name, fk)
                fkmm = MFKMode(name)
                allPkList = fkmm.get('pklist')
                if allPkList and pk in allPkList:
                    allPkList.remove(pk)
                    fkmm.update('pklist', allPkList)  # 删除主键
        ...
        if mm:
            ...
            if self._fk:
                ...
                if data:
                    ...
                    pklist = fkmm.get('pklist')
                    if pklist is None:
                        pklist = self.getAllPkByFk(fk)  # add by yzw 修复一开始删除时候没有外键导致主键列表重复
    ...
    def new(self, data, isSyncNow=False):
        ...
        if self._fk:
            ...
            if pklist is None:
                pklist = self.getAllPkByFk(fk)
            if pk not in pklist:  # add by yzw 防止主键重复
                pklist.append(pk)
            fkmm.update('pklist', pklist)
